...
function createModerationService({ db, presence, chat, controlChannel, environment = process.env }) {
  ...
  return {
        ...
        await client.query(
          `INSERT INTO notifications (user_id, type, title, body, data)
           VALUES ($1, 'account_ban', 'Account suspended', $2,
                   jsonb_build_object('type', $3::text, 'startsAt', $4::timestamptz, 'endsAt', $5::timestamptz))`,
          [targetUserId, normalizedReason, banType, inserted.rows[0].starts_at, inserted.rows[0].ends_at]
        );
    ...
    async banGuest({ actorUserId, targetGuestId, type, reason, hours, correlationId = crypto.randomUUID() }) {
      ...
      if (!outcome.idempotent) await endAndDisconnectGuest(targetGuestId, { type: outcome.ban.type, endsAt: outcome.ban.ends_at });
      return { ...outcome.ban, idempotent: outcome.idempotent, correlationId };
    },
