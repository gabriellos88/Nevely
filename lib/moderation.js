...
function createModerationService({ db, presence, chat, environment = process.env }) {
  ...
  return {
    ...
    async createNetworkBan({ actorUserId, cidr, reason, hours, privacyReviewedByUserId, privacyReviewReference, correlationId = crypto.randomUUID() }) {
      correlationId = normalizedCorrelationId(correlationId);
      if (!privacyReviewedByUserId || privacyReviewedByUserId === actorUserId) {
        throw new ModerationError('PRIVACY_REVIEW_REQUIRED', 'An independent administrator privacy review is required');
      }
      const normalizedReason = requiredReason(reason);
      ...
      const reviewReference = String(privacyReviewReference || '').trim();
      if (reviewReference.length < 3 || reviewReference.length > 160) {
      ...
      return withTransaction(db, async (client) => {
        const reviewer = await client.query(
          `SELECT 1 FROM users WHERE id = $1 AND role = 'admin' AND deleted_at IS NULL`, [privacyReviewedByUserId]
        );
        ...
        const inserted = await client.query(
          ...
          [networkFingerprint, network.family, network.prefix, normalizedReason, durationHours, actorUserId,
            privacyReviewedByUserId, reviewReference, correlationId]
        );
        await appendAudit(client, {
