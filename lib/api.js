...
function registerApiRoutes(app, db, presence, options = {}) {
  ...
  async function guestDevice(req, res) {
    ...
    if (await moderation?.isGuestDeviceRestricted(fingerprint)) {
      return false;
    }
    return fingerprint;
  }
  ...
  function guestDeviceRestricted(res) {
    return res.status(403).json({ error: copy.errors.guestRestricted, code: 'GUEST_DEVICE_RESTRICTED' });
  }
  ...
  app.get('/admin', requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    res.render('admin', {
      ...
      adminReauthMethod,
      adminReauthActive: Number(req.session.adminReauthenticatedAt || 0) + 10 * 60 * 1000 > Date.now(),
