...
function registerApiRoutes(app, db, presence, options = {}) {
    ...
    res.json({
      users: paged.items.map((user) => {
        ...
        return {
          ...
          display_name: deleted ? copy.common.deletedUser : user.display_name,
          email: deleted ? null : user.email,
          role: user.role,
          plan: user.plan,
          email_verified_at: deleted ? null : user.email_verified_at,
          admin_2fa_enabled_at: deleted ? null : user.admin_2fa_enabled_at,
          created_at: user.created_at,
          deleted_at: user.deleted_at,
          scheduled_deletion: null,
          age: deleted ? null : Number(user.age),
          country: deleted ? null : user.country,
          country_code: deleted ? null : user.country_code,
  ...
  app.get('/api/admin/users/:id', requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    res.json({
      user: {
        ...
        displayName: deleted ? copy.common.deletedUser : row.display_name,
        email: deleted ? null : row.email,
        role: row.role,
        plan: row.plan,
        emailVerifiedAt: deleted ? null : row.email_verified_at,
        adminTwoFactorEnabledAt: deleted ? null : row.admin_2fa_enabled_at,
        createdAt: row.created_at,
        deletedAt: row.deleted_at,
        scheduledDeletion: null,
        age: deleted ? null : Number(row.age),
        country: deleted ? null : row.country,
        countryCode: deleted ? null : row.country_code,
  ...
  function moderationFailure(error, res) {
    ...
    if (['MODERATION_REASON_REQUIRED', 'NETWORK_INVALID', 'NETWORK_TOO_BROAD', 'BAN_TYPE_INVALID', 'BAN_DURATION_INVALID', 'GUEST_BAN_TYPE_INVALID', 'GUEST_BAN_DURATION_INVALID'].includes(error?.code)) return res.status(400).json({ error: error.message });
