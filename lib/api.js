  ...
  app.get('/api/conversations', asyncRoute(async (req, res) => {
      ...
      principal.userId
        ? db.query(
          `SELECT c.id, c.public_id, c.type, c.status, c.started_at, c.ended_at, c.expires_at,
           ...
           LIMIT 5`,
          [principal.userId, copy.common.unknownUser]
        )
    ...
    const conversations = paged.items.map((item) => ({
      ...
      capabilities: {
        ...
        canResumeDirect: Boolean(principal.userId && item.type === 'direct' && item.status === 'active'),
    ...
    const directInbox = directInboxRows.rows.map((item) => ({
      ...
      capabilities: {
        ...
        canResumeDirect: item.status === 'active',
        canAddFriend: socialCapabilities.get(item.partner_public_id)?.canAddFriend === true,
        canBlock: socialCapabilities.get(item.partner_public_id)?.canBlock === true
      }
    }));
  ...
  app.get('/api/saved-chats', asyncRoute(async (req, res) => {
    ...
    const result = await db.query(
      `SELECT s.conversation_id, c.public_id AS conversation_public_id,
               ...
               COALESCE(partner.account_name, partner.guest_name, partner.display_name, $3) AS partner_name,
               COALESCE(partner.public_id, partner.guest_public_id) AS partner_public_id,
    ...
    res.json({
      chats: result.rows.map((chat) => ({
        ...
        capabilities: {
          ...
          canResumeDirect: Boolean(principal.userId && chat.type === 'direct' && chat.status === 'active'),
