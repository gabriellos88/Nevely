...
function registerApiRoutes(app, db, presence, options = {}) {
  ...
  app.get('/api/admin/guests', requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const result = await db.query(
      `SELECT g.id, g.display_alias, g.name, g.gender, g.age, g.country, g.country_code,
       ...
       WHERE ($1::text IS NULL OR g.status = $1)
         AND ($2::text = '' OR LOWER(g.display_alias) LIKE $2::text || '%' ESCAPE '\\'
              OR LOWER(g.name) LIKE $2::text || '%' ESCAPE '\\' OR g.id::text = $2::text)
         AND ($3::timestamptz IS NULL OR (g.created_at, g.id) < ($3::timestamptz, $4::uuid))
       ORDER BY g.created_at DESC, g.id DESC
       LIMIT $5`,
      [status, escapedSearch, cursor?.createdAt || null, cursor?.id || null, limit + 1]
    );
  ...
  app.get('/api/admin/users', requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const result = await db.query(
      `SELECT u.id, u.public_id, u.username, u.display_alias, u.display_name, u.email, u.role, u.plan,
              ...
              latest_activity.last_seen_at, latest_activity.recent_chat_count,
              active_ban.id AS active_ban_id,
              EXISTS (
  ...
  app.get('/api/admin/guests/:id', requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    const guestId = String(req.params.id || '').toLowerCase();
    if (!UUID_PATTERN.test(guestId)) return res.status(404).json({ error: copy.errors.guestProfileMissing });
    const result = await db.query(
      `SELECT g.id, g.display_alias, g.name, g.gender, g.age, g.country, g.country_code, g.avatar_id,
              g.status, g.created_at, g.last_seen_at, g.retention_until, g.deleted_at,
              active_ban.id AS active_ban_id, active_ban.reason AS active_ban_reason,
              active_ban.ends_at AS active_ban_ends_at, latest_activity.recent_chat_count
       FROM guest_principals g
       LEFT JOIN LATERAL (
         SELECT id, reason, ends_at FROM guest_bans b
         WHERE b.guest_id = g.id AND b.revoked_at IS NULL
           AND b.starts_at <= NOW() AND b.ends_at > NOW()
         ORDER BY b.created_at DESC, b.id DESC LIMIT 1
       ) active_ban ON TRUE
    ...
    res.json({ guest: {
      id: row.id, displayAlias: row.display_alias, name: row.name, gender: row.gender,
      ...
      activeBan: row.active_ban_id ? {
        id: Number(row.active_ban_id), reason: row.active_ban_reason, endsAt: row.active_ban_ends_at
      } : null
    } });
  }));
  ...
  app.get('/api/admin/bans', requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const result = await db.query(
      `WITH moderation_bans AS (
         ...
         JOIN users creator ON creator.id = b.created_by
         UNION ALL
         SELECT -1000000000000 - b.id AS id, b.id AS ban_id, 'temporary'::varchar(20) AS type,
                b.reason, b.starts_at, b.ends_at, b.created_at, b.revoked_at, 'guest'::text AS scope,
                NULL::varchar(40) AS user_public_id, target.display_alias AS user_name,
                creator.public_id AS created_by_public_id
         FROM guest_bans b
  ...
  function moderationFailure(error, res) {
    ...
    if (['MODERATION_REASON_REQUIRED', 'NETWORK_INVALID', 'NETWORK_TOO_BROAD', 'BAN_TYPE_INVALID', 'BAN_DURATION_INVALID', 'GUEST_BAN_DURATION_INVALID'].includes(error?.code)) return res.status(400).json({ error: error.message });
  ...
  app.post('/api/admin/guests/:id/ban', ...highRiskAdmin, asyncRoute(async (req, res) => {
    const targetGuestId = String(req.params.id || '').toLowerCase();
    try {
      const ban = await moderation.banGuest({
        actorUserId: userId(req), targetGuestId, hours: req.body.hours, reason: req.body.reason,
        correlationId: req.get('x-correlation-id') || undefined
      });
      return res.status(ban.idempotent ? 200 : 201).json({
        banId: Number(ban.id), endsAt: ban.ends_at, idempotent: ban.idempotent
      });
    } catch (error) {
      return moderationFailure(error, res);
    }
  ...
  app.post('/api/admin/network-ban-privacy-approvals', ...highRiskAdmin, asyncRoute(async (req, res) => {
    try {
      const approval = await moderation.requestNetworkBanPrivacyApproval({
        actorUserId: userId(req), cidr: req.body.cidr, reason: req.body.reason,
        correlationId: req.get('x-correlation-id') || undefined
      });
  ...
  app.post('/api/admin/network-ban-privacy-approvals/:id/approve', ...highRiskAdmin, asyncRoute(async (req, res) => {
    try {
      const approval = await moderation.approveNetworkBanPrivacyApproval({
        reviewerUserId: userId(req), approvalId: req.params.id, reason: req.body.reason,
        reviewReference: req.body.reviewReference,
        correlationId: req.get('x-correlation-id') || undefined
      });
      return res.json({ approvalId: approval.id, expiresAt: approval.expires_at });
    } catch (error) {
      return moderationFailure(error, res);
    }
  }));
  ...
  app.post('/api/admin/network-bans', ...highRiskAdmin, asyncRoute(async (req, res) => {
    try {
      const ban = await moderation.createNetworkBan({
        ...
        privacyApprovalId: req.body.privacyApprovalId,
        correlationId: req.get('x-correlation-id') || undefined
      });
      return res.status(ban.idempotent ? 200 : 201).json({
