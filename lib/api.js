...
const { MAX_VISIBLE_CONVERSATION_MESSAGES } = require('./conversation-messages');
const {
  bindGuestDevice,
  bindGuestSession,
  clearGuestSession,
...
function notificationDataForClient(notification) {
  ...
  if (notification.type === 'chat_request' && isPublicId(data.requestPublicId, 'chatRequest')) {
    return {
      requestPublicId: data.requestPublicId,
      ...(isPublicId(data.userPublicId, 'user') ? { userPublicId: data.userPublicId } : {})
    };
...
async function productPrincipalForRequest(db, req, res) {
  ...
  if (req.session.user) {
    ...
    return {
      ...
      guestId: null,
      savedChatLimit: req.session.user.plan === 'premium' ? 10 : 5
    };
  }
...
async function conversationIdFromPublicId(executor, value) {
  const publicId = cleanPublicId(value);
  if (!isPublicId(publicId, 'conversation')) return null;
  const result = await executor.query(
    'SELECT id FROM conversations WHERE public_id = $1 LIMIT 1',
    [publicId]
  );
  return result.rowCount ? Number(result.rows[0].id) : null;
}

async function conversationSocialCapabilities(db, principal, rows) {
  const publicIds = [...new Set(rows
    .map((row) => row.partner_public_id)
    .filter((publicId) => typeof publicId === 'string'))];
  if (!principal.userId || publicIds.length === 0) return new Map();
  const result = await db.query(
    `SELECT target.public_id,
            target.email_verified_at IS NOT NULL
              AND target.deleted_at IS NULL
              AND NOT EXISTS (
                SELECT 1 FROM account_bans ban
                WHERE ban.user_id = target.id AND ban.revoked_at IS NULL
                  AND ban.starts_at <= NOW()
                  AND (ban.type = 'permanent' OR ban.ends_at > NOW())
              )
              AND NOT EXISTS (
                SELECT 1 FROM friendships friendship
                WHERE (friendship.user_id = $1 AND friendship.friend_id = target.id)
                   OR (friendship.user_id = target.id AND friendship.friend_id = $1)
              )
              AND NOT EXISTS (
                SELECT 1 FROM blocked_users block
                WHERE (block.blocker_user_id = $1 AND block.blocked_user_id = target.id)
                   OR (block.blocker_user_id = target.id AND block.blocked_user_id = $1)
              )
              AND NOT EXISTS (
                SELECT 1 FROM friend_requests request
                WHERE request.status = 'pending'
                  AND LEAST(request.sender_user_id, request.receiver_user_id) = LEAST($1::bigint, target.id)
                  AND GREATEST(request.sender_user_id, request.receiver_user_id) = GREATEST($1::bigint, target.id)
              ) AS can_add_friend,
       ...
       FROM users target
       WHERE target.public_id = ANY($2::text[])
         AND target.id <> $1`,
    [principal.userId, publicIds]
  );
  return new Map(result.rows.map((row) => [row.public_id, {
    canAddFriend: Boolean(row.can_add_friend),
    canBlock: Boolean(row.can_block)
  }]));
}
...
function registerApiRoutes(app, db, presence, options = {}) {
  ...
  const productRateLimiter = options.rateLimiter || createModerationRateLimiter({ db });
  ...
  async function endDirectConversationForPair(executor, firstUserId, secondUserId) {
    ...
    return ended.rows.map((row) => Number(row.id));
  }
  ...
  app.get('/api/users/:id/profile', ...auth, asyncRoute(async (req, res) => {
    ...
    if (req.query.context === 'history') {
      const conversationId = await conversationIdFromPublicId(db, req.query.conversationId);
      if (!conversationId) return res.status(404).json({ error: copy.errors.conversationUnavailable });
      const historyAccess = await db.query(
        `SELECT 1 FROM conversation_participants mine
         JOIN conversation_participants partner
           ON partner.conversation_id = mine.conversation_id
          AND partner.user_id = $2
         WHERE mine.conversation_id = $3 AND mine.user_id = $1
         LIMIT 1`,
        [userId(req), targetId, conversationId]
      );
      if (!historyAccess.rowCount) return res.status(404).json({ error: copy.errors.conversationUnavailable });
      presenceVisible = false;
    }
    res.json({
      user: result.rows[0],
      presenceVisible,
      ...(presenceVisible ? { online: presence.isOnline(targetId) } : {})
    });
  }));
  ...
  app.get('/api/conversations', asyncRoute(async (req, res) => {
    ...
    const cursor = req.query.cursor
      ? decodePublicIdCursor(req.query.cursor, 'conversation')
      : null;
    ...
    const [result, unread] = await Promise.all([
      db.query(
      `SELECT c.public_id, c.public_id AS id, c.type, c.status, c.started_at, c.ended_at, c.expires_at,
               c.created_at, c.last_activity_at,
               CASE WHEN partner.account_available OR partner.guest_available
                 THEN COALESCE(partner.account_name, partner.guest_name, partner.display_name, $3)
                 ELSE $7
               END AS partner_name,
               CASE WHEN partner.account_available THEN partner.public_id
                    WHEN partner.guest_available THEN partner.guest_public_id
                    ELSE NULL
               END AS partner_public_id,
               CASE WHEN partner.account_available THEN partner.profile_image_url ELSE NULL END AS profile_image_url,
               NOT COALESCE(partner.account_available OR partner.guest_available, FALSE) AS partner_anonymized,
               EXISTS(
        ...
        LEFT JOIN LATERAL (
          SELECT cp.user_id, cp.guest_id, u.public_id, guest.public_id AS guest_public_id,
                 ...
                 COALESCE(u.deleted_at IS NULL, FALSE) AS account_available,
                 COALESCE(guest.status = 'active' AND guest.retention_until > NOW(), FALSE) AS guest_available
        ...
        WHERE (mine.user_id = $1 OR mine.guest_id = $2)
          ...
          AND (c.status = 'active' OR NOT EXISTS (
            SELECT 1 FROM conversation_history_visibility visibility
            WHERE visibility.conversation_id = c.id
              AND (visibility.user_id = $1 OR visibility.guest_id = $2)
          ))
           AND (c.status = 'active' OR c.expires_at > NOW() OR EXISTS (SELECT 1 FROM saved_chats s2 WHERE s2.conversation_id = c.id))
           AND (
             $4::timestamptz IS NULL
             OR (c.created_at, c.public_id) < ($4::timestamptz, $5::text)
           )
      ...
      [
        ...
        limit + 1,
        copy.common.unavailableParticipant
      ]
      ),
      db.query(
        `SELECT COUNT(*)::int AS count
         ...
         WHERE (mr.user_id = $1 OR mr.guest_id = $2)
            ...
            AND c.deleted_for_everyone_at IS NULL
           AND (c.status = 'active' OR c.expires_at > NOW() OR EXISTS (
              SELECT 1 FROM saved_chats s WHERE s.conversation_id = c.id
            ))`,
        [principal.userId, principal.guestId]
      )
    ]);
    const paged = publicIdCursorPage(result.rows, limit, 'conversation');
    const [savedUsage, directInboxRows, pendingChatRequests] = await Promise.all([
      db.query(
      ...
      principal.userId
        ? db.query(
          `SELECT c.public_id, c.public_id AS id, c.type, c.status, c.started_at, c.ended_at, c.expires_at,
                  c.created_at, c.last_activity_at,
                  CASE WHEN partner.available THEN COALESCE(partner.display_name, $2) ELSE $3 END AS partner_name,
                  CASE WHEN partner.available THEN partner.public_id ELSE NULL END AS partner_public_id,
                  CASE WHEN partner.available THEN partner.profile_image_url ELSE NULL END AS profile_image_url,
                  NOT COALESCE(partner.available, FALSE) AS partner_anonymized,
                  EXISTS (SELECT 1 FROM saved_chats s WHERE s.conversation_id = c.id AND s.user_id = $1) AS saved,
                  (SELECT body FROM messages m WHERE m.conversation_id = c.id AND m.deleted_for_everyone_at IS NULL ORDER BY m.created_at DESC LIMIT 1) AS last_message,
                  (SELECT COUNT(*)::int FROM message_receipts mr
                   JOIN messages unread_message ON unread_message.id = mr.message_id
           ...
           LEFT JOIN LATERAL (
             SELECT u.public_id, u.display_name, u.profile_image_url,
                    u.deleted_at IS NULL AS available
           ...
           WHERE cp.user_id = $1 AND c.type = 'direct'
             ...
             AND (c.status = 'active' OR NOT EXISTS (
               SELECT 1 FROM conversation_history_visibility visibility
               WHERE visibility.conversation_id = c.id AND visibility.user_id = $1
             ))
             AND (c.status = 'active' OR c.expires_at > NOW() OR EXISTS (
               SELECT 1 FROM saved_chats s WHERE s.conversation_id = c.id
             ))
           ORDER BY (c.status = 'active') DESC, c.last_activity_at DESC, c.id DESC
           LIMIT 5`,
          [principal.userId, copy.common.unknownUser, copy.common.unavailableParticipant]
        )
        : Promise.resolve({ rows: [] }),
      principal.userId
        ? db.query(
          `SELECT COUNT(*)::int AS count FROM chat_requests
           WHERE receiver_user_id = $1 AND status = 'pending' AND expires_at > NOW()`,
          [principal.userId]
        )
        : Promise.resolve({ rows: [{ count: 0 }] })
    ]);
    const savedCount = Number(savedUsage.rows[0]?.count) || 0;
    const socialCapabilities = await conversationSocialCapabilities(
      db,
      principal,
      [...paged.items, ...directInboxRows.rows]
    );
    const conversations = paged.items.map((item) => ({
      ...item,
      capabilities: {
        canSave: !item.saved && savedCount < principal.savedChatLimit,
        canUnsave: Boolean(item.saved),
        canRemoveFromHistory: item.status !== 'active',
        canDeleteUnsavedMessages: Boolean(principal.userId && item.status !== 'active'),
        canResumeDirect: Boolean(principal.userId && item.partner_public_id
          && item.type === 'direct' && item.status === 'active'),
        canViewPartnerProfile: Boolean(item.partner_public_id),
        canAddFriend: socialCapabilities.get(item.partner_public_id)?.canAddFriend === true,
        canBlock: socialCapabilities.get(item.partner_public_id)?.canBlock === true
      }
    }));
    const directInbox = directInboxRows.rows.map((item) => ({
      ...item,
      capabilities: {
        canSave: !item.saved && savedCount < principal.savedChatLimit,
        canUnsave: Boolean(item.saved),
        canRemoveFromHistory: item.status !== 'active',
        canDeleteUnsavedMessages: item.status !== 'active',
        canResumeDirect: Boolean(item.partner_public_id && item.status === 'active'),
        canViewPartnerProfile: Boolean(item.partner_public_id),
        canAddFriend: socialCapabilities.get(item.partner_public_id)?.canAddFriend === true,
        canBlock: socialCapabilities.get(item.partner_public_id)?.canBlock === true
      }
    ...
    res.json({
      conversations,
      directInbox: {
        active: directInbox.filter((item) => item.status === 'active'),
        recent: directInbox.filter((item) => item.status !== 'active'),
        limit: 5
      },
      page: paged.page,
      unreadCount: Number(unread.rows[0].count),
      pendingChatRequestCount: Number(pendingChatRequests.rows[0]?.count) || 0,
      messageBadgeCount: Number(unread.rows[0].count) + (Number(pendingChatRequests.rows[0]?.count) || 0),
      limits: {
  ...
  app.get('/api/conversations/:id/messages', asyncRoute(async (req, res) => {
    const principal = await productPrincipalForRequest(db, req, res);
    if (!principal) return;
    const conversationPublicId = cleanPublicId(req.params.id);
    const conversationId = await conversationIdFromPublicId(db, conversationPublicId);
    const limit = pageSize(req.query.limit);
    const beforeMessageId = req.query.beforeMessageId
      ? cleanPublicId(req.query.beforeMessageId)
      : null;
    if (!conversationId) {
      return res.status(404).json({ error: copy.errors.conversationUnavailable });
    }
    if (req.query.beforeMessageId && !isPublicId(beforeMessageId, 'message')) {
      return res.status(400).json({ error: copy.errors.paginationInvalid });
    }
    const allowed = await db.query(
      `SELECT c.public_id AS id, c.public_id, c.status, c.started_at, c.ended_at,
               EXISTS(
                 SELECT 1 FROM saved_chats s
                 WHERE s.conversation_id = c.id
                   AND (s.user_id = $1 OR s.guest_id = $2)
    ...
    const messages = await db.query(
      `SELECT m.public_id AS id, m.public_id,
               CASE
                 WHEN COALESCE(m.sender_user_id = $4, FALSE) THEN sender.public_id
                 WHEN COALESCE(m.sender_guest_id = $5, FALSE) THEN sender_guest.public_id
                 WHEN sender.deleted_at IS NULL THEN sender.public_id
                 WHEN sender_guest.status = 'active' AND sender_guest.retention_until > NOW() THEN sender_guest.public_id
                 ELSE NULL
               ...
               CASE
                 WHEN COALESCE(m.sender_user_id = $4, FALSE)
                   OR COALESCE(m.sender_guest_id = $5, FALSE)
                   OR sender.deleted_at IS NULL
                   OR (sender_guest.status = 'active' AND sender_guest.retention_until > NOW())
                 THEN m.sender_display_name
                 ELSE $7
        ...
        WHERE m.conversation_id = $1 AND m.deleted_for_everyone_at IS NULL
          AND ($2::text IS NULL OR m.id < (
            SELECT boundary.id FROM messages boundary
            WHERE boundary.public_id = $2 AND boundary.conversation_id = $1
          ))
          AND m.id >= COALESCE((
            SELECT MIN(visible.id)
            FROM (
              SELECT newest.id
              FROM messages newest
              WHERE newest.conversation_id = $1
                AND newest.deleted_for_everyone_at IS NULL
  ...
  app.patch('/api/conversations/:id/read', asyncRoute(async (req, res) => {
    ...
    const conversationPublicId = cleanPublicId(req.params.id);
    const conversationId = await conversationIdFromPublicId(db, conversationPublicId);
    const upToMessageId = cleanPublicId(req.body.upToMessageId);
    if (!conversationId) {
      return res.status(404).json({ error: copy.errors.conversationUnavailable });
    }
    if (!isPublicId(upToMessageId, 'message')) {
    ...
    res.json({ updated: result.rowCount, conversationId: conversationPublicId, upToMessageId, readAt });
  }));

  app.delete('/api/conversations/:publicId/history', asyncRoute(async (req, res) => {
    if (req.body.confirmation !== 'REMOVE FROM MY HISTORY') {
      return res.status(400).json({ error: copy.errors.confirmationRequired });
    }
    const principal = await productPrincipalForRequest(db, req, res);
    if (!principal) return;
    const conversationPublicId = cleanPublicId(req.params.publicId);
    if (!isPublicId(conversationPublicId, 'conversation')) {
    ...
    try {
      ...
      const conversation = await client.query(
        `SELECT c.id, c.status
         ...
         WHERE c.public_id = $1
           AND (cp.user_id = $2 OR cp.guest_id = $3)
           AND c.deleted_for_everyone_at IS NULL
         FOR UPDATE OF c`,
        [conversationPublicId, principal.userId, principal.guestId]
      );
      if (!conversation.rowCount || conversation.rows[0].status === 'active') {
        await client.query('ROLLBACK');
        return res.status(404).json({ error: copy.errors.conversationNotFound });
      }
      const conversationId = Number(conversation.rows[0].id);
      const hidden = await client.query(
        `INSERT INTO conversation_history_visibility (conversation_id, user_id, guest_id)
         SELECT $1, $2, $3
         WHERE NOT EXISTS (
           SELECT 1 FROM conversation_history_visibility visibility
           WHERE visibility.conversation_id = $1
             AND (visibility.user_id = $2 OR visibility.guest_id = $3)
         )`,
        [conversationId, principal.userId, principal.guestId]
      );
      await client.query(
        `DELETE FROM saved_chats
         WHERE conversation_id = $1 AND (user_id = $2 OR guest_id = $3)`,
        [conversationId, principal.userId, principal.guestId]
      );
      if (hidden.rowCount > 0) {
        await appendAudit(client, {
          actorUserId: principal.userId,
          targetGuestId: principal.guestId,
          targetType: 'conversation',
          action: 'product.conversation_history_removed',
          before: { conversationId, visible: true },
          after: { conversationId, visible: false },
  ...
  app.post('/api/conversations/:publicId/delete-unsaved', ...auth, asyncRoute(async (req, res) => {
    if (req.body.confirmation !== 'DELETE UNSAVED MESSAGES') {
      return res.status(400).json({ error: copy.errors.confirmationRequired });
    }
    const conversationPublicId = cleanPublicId(req.params.publicId);
    if (!isPublicId(conversationPublicId, 'conversation')) {
    ...
    const rate = await productRateLimiter.consume({
      principalType: 'user',
      principalId: String(userId(req)),
      action: 'conversation-delete-unsaved'
    });
    if (!rate.allowed) {
    ...
    try {
      ...
      const conversation = await client.query(
        `SELECT c.id, c.status
         FROM conversations c
         JOIN conversation_participants cp ON cp.conversation_id = c.id
         WHERE c.public_id = $1 AND cp.user_id = $2
           AND c.deleted_for_everyone_at IS NULL
         FOR UPDATE OF c`,
        [conversationPublicId, userId(req)]
      );
      if (!conversation.rowCount || conversation.rows[0].status === 'active') {
        await client.query('ROLLBACK');
        return res.status(404).json({ error: copy.errors.conversationNotFound });
      ...
      const deleted = await client.query(
        `UPDATE messages message
         SET deleted_for_everyone_at = COALESCE(message.deleted_for_everyone_at, NOW())
         WHERE message.conversation_id = $1
           AND message.deleted_for_everyone_at IS NULL
           AND NOT EXISTS (
             SELECT 1 FROM saved_chats saved WHERE saved.conversation_id = $1
           )
           AND NOT EXISTS (
             SELECT 1 FROM reports report
             WHERE report.conversation_id = $1
               AND (report.retention_until IS NULL OR report.retention_until > NOW())
           )`,
        [Number(conversation.rows[0].id)]
      );
      if (deleted.rowCount > 0) {
        ...
        await appendAudit(client, {
          ...
          targetType: 'conversation',
          action: 'product.unsaved_messages_deleted',
          before: { conversationId, deletedMessageCount: 0 },
          after: { conversationId, deletedMessageCount: deleted.rowCount },
          correlationId: req.get('x-correlation-id')
        });
      }
      await client.query('COMMIT');
  ...
  app.get('/api/saved-chats', asyncRoute(async (req, res) => {
    ...
    if (!principal) return;
    const result = await db.query(
      `SELECT c.public_id AS conversation_id, c.public_id AS conversation_public_id,
               s.created_at, c.started_at, c.type, c.status,
               CASE WHEN partner.account_available OR partner.guest_available
                 THEN COALESCE(partner.account_name, partner.guest_name, partner.display_name, $3)
                 ELSE $4
               END AS partner_name,
        ...
        LEFT JOIN LATERAL (
          SELECT u.display_name AS account_name, guest.name AS guest_name, cp.display_name,
                 u.public_id, guest.public_id AS guest_public_id, u.profile_image_url,
                 COALESCE(u.deleted_at IS NULL, FALSE) AS account_available,
                 COALESCE(guest.status = 'active' AND guest.retention_until > NOW(), FALSE) AS guest_available
          FROM conversation_participants cp
          LEFT JOIN users u ON u.id = cp.user_id
      ...
      [principal.userId, principal.guestId, copy.common.guest, copy.common.unavailableParticipant]
    );
    const socialCapabilities = await conversationSocialCapabilities(db, principal, result.rows);
    res.json({
      chats: result.rows.map((chat) => ({
        ...
        capabilities: {
          canSave: false,
          canUnsave: true,
          canRemoveFromHistory: chat.status !== 'active',
          canDeleteUnsavedMessages: Boolean(principal.userId && chat.status !== 'active'),
          canResumeDirect: Boolean(principal.userId && chat.partner_public_id
            && chat.type === 'direct' && chat.status === 'active'),
          canViewPartnerProfile: Boolean(chat.partner_public_id),
          canAddFriend: socialCapabilities.get(chat.partner_public_id)?.canAddFriend === true,
          canBlock: socialCapabilities.get(chat.partner_public_id)?.canBlock === true
        }
      })),
  ...
  app.put('/api/conversations/:id/saved', asyncRoute(async (req, res) => {
    ...
    const conversationId = await conversationIdFromPublicId(db, req.params.id);
  ...
  app.delete('/api/conversations/:id/saved', asyncRoute(async (req, res) => {
    ...
    if (!principal) return;
    const conversationId = await conversationIdFromPublicId(db, req.params.id);
    if (!conversationId) return res.status(204).end();
    await db.query(
      `DELETE FROM saved_chats
       WHERE conversation_id = $1
         AND (user_id = $2 OR guest_id = $3)`,
  ...
  app.get('/api/friends', ...auth, asyncRoute(async (req, res) => {
    ...
    const result = await db.query(
      `SELECT f.created_at, u.id AS internal_id, u.public_id,
              ...
              (
                ...
                WHERE pair.user_low_id = LEAST($1::bigint, u.id)
                  AND pair.user_high_id = GREATEST($1::bigint, u.id)
                  AND c.type = 'direct' AND c.status = 'active'
                  AND c.deleted_for_everyone_at IS NULL
                LIMIT 1
              ) AS active_direct_conversation_id
    ...
    res.json({
      friends: paged.items.map(({
        ...
        banned,
        blocked,
        active_direct_conversation_id: activeDirectConversationId,
        ...friend
      }) => ({
        ...friend,
        online: presence.isOnline(internalId),
        capabilities: {
          canStartDirectChat: Boolean(emailVerifiedAt && !banned && !blocked && !activeDirectConversationId),
          canOpenDirectChat: Boolean(activeDirectConversationId),
          canRemoveFriend: true,
          canBlock: !blocked,
          activeDirectConversationId: activeDirectConversationId || null
        }
      })),
      page: paged.page
    });
  ...
  app.delete('/api/friends/:id', ...auth, asyncRoute(async (req, res) => {
    ...
    const client = await db.getClient();
    let changed = false;
    let endedDirectConversationIds = [];
    try {
      await client.query('BEGIN');
      ...
      endedDirectConversationIds = await endDirectConversationForPair(client, ownerId, friendId);
      if (changed) {
        await appendAudit(client, {
          actorUserId: ownerId,
          targetUserId: friendId,
          targetType: 'account',
          action: 'social.friendship_removed',
          before: { friends: true },
          after: { friends: false },
          correlationId: req.get('x-correlation-id')
        });
    ...
    if (endedDirectConversationIds.length) {
      await options.chat?.terminateDirectPair?.(ownerId, friendId, { persist: false });
    }
    return res.status(204).end();
  }));
  ...
  app.get('/api/notifications', asyncRoute(async (req, res) => {
    ...
    const [result, unread] = await Promise.all([
      db.query(
      `SELECT notification.public_id, notification.public_id AS id,
       ...
       LEFT JOIN LATERAL (
         SELECT users.public_id, users.display_name, users.profile_image_url
         FROM users
         WHERE users.deleted_at IS NULL
           AND users.public_id = COALESCE(
             NULLIF(notification.data->>'userPublicId', ''),
             (
               SELECT sender.public_id
               FROM chat_requests request
               JOIN users sender ON sender.id = request.sender_user_id
               WHERE request.public_id = notification.data->>'requestPublicId'
        ...
        WHERE (notification.user_id = $1 OR notification.guest_id = $2)
         AND notification.type <> 'account_ban'
         AND notification.dismissed_at IS NULL
         AND (
            $3::timestamptz IS NULL
            OR (notification.created_at, notification.public_id) < ($3::timestamptz, $4::text)
         )
      ...
      notifications: paged.items.map((notification) => {
        ...
        return {
          ...
          actor: actorPublicId ? {
            ...
            profileImageUrl: actorProfileImageUrl || null
          } : null
        };
      }),
      page: paged.page,
  ...
  app.delete('/api/notifications/:id', asyncRoute(async (req, res) => {
    ...
    const updated = await db.query(
      `UPDATE notifications
       ...
       RETURNING user_id`,
      [notificationPublicId, principal.userId, principal.guestId,
        ['friend_request', 'friend_accepted', 'chat_request', 'report_processed', 'guest_account_claim']]
    );
    if (updated.rows[0]?.user_id) {
      presence.emitToUser(Number(updated.rows[0].user_id), 'notification-updated', {
        notificationId: notificationPublicId,
  ...
  app.put('/api/blocks/:id', ...auth, asyncRoute(async (req, res) => {
    ...
    try {
      ...
      endedDirectConversationIds = await endDirectConversationForPair(client, blockerId, blockedId);
      if (created) {
        await appendAudit(client, {
          actorUserId: blockerId,
          targetUserId: blockedId,
          targetType: 'account',
          action: 'social.account_blocked',
    ...
    if (endedDirectConversationIds.length) {
      await options.chat?.terminateDirectPair?.(blockerId, blockedId, { persist: false });
    }
    ...
    try {
      ...
      if (removed.rowCount > 0) {
        await appendAudit(client, {
          ...
          targetUserId: blockedId,
          targetType: 'account',
          action: 'social.account_unblocked',
          before: { blocked: true },
          after: { blocked: false },
          correlationId: req.get('x-correlation-id')
        });
