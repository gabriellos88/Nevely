...
function registerApiRoutes(app, db, presence, options = {}) {
  ...
  app.post('/api/guest-profile', asyncRoute(async (req, res) => {
    ...
    if (db.isConfigured) {
      ...
      if (existing) {
        await saveSession(req);
        return res.json({ guest: existing });
      }
  ...
  app.get('/api/conversations', ...auth, asyncRoute(async (req, res) => {
    ...
    const [result, unread] = await Promise.all([
      ...
      `SELECT c.id, c.type, c.status, c.started_at, c.ended_at, c.expires_at,
       ...
       LIMIT $5`,
      [userId(req), copy.common.guest, cursor?.createdAt || null, cursor?.id || null, limit + 1]
      ),
      db.query(
  ...
  app.get('/api/conversations/:id/messages', ...auth, asyncRoute(async (req, res) => {
    ...
    const messages = await db.query(
      `SELECT m.id, sender.public_id AS sender_public_id, m.sender_display_name, m.body,
       ...
       ORDER BY m.id DESC
       LIMIT $3`,
      [conversationId, beforeMessageId, limit + 1]
    );
  ...
  app.get('/api/admin/users', requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const result = await db.query(
      `SELECT id, public_id, display_alias, display_name, email, role, plan,
              email_verified_at, admin_2fa_enabled_at, created_at
       FROM users
       WHERE deleted_at IS NULL
         AND (
           $1::text = ''
           OR LOWER(username) LIKE $1::text || '%' ESCAPE '\\'
           OR LOWER(email) LIKE $1::text || '%' ESCAPE '\\'
           OR public_id = $1::text
         )
         AND (
           $2::timestamptz IS NULL
       ...
       ORDER BY created_at DESC, id DESC
       LIMIT $4`,
      [escapedSearch, cursor?.createdAt || null, cursor?.id || null, limit + 1]
    );
    const paged = cursorPage(result.rows, limit);
    res.json({
      users: paged.items.map(({ id, ...user }) => user),
  ...
  app.get('/api/admin/reports', requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const { cursor, limit } = pagination;
    const result = await db.query(
      `SELECT r.id, r.reason, r.details, r.status, r.resolution,
       ...
       WHERE (
         ...
         OR (r.created_at, r.id) < ($1::timestamptz, $2::bigint)
       )
  ...
  app.get('/api/admin/bans', requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const { cursor, limit } = pagination;
    const result = await db.query(
      `SELECT b.id, b.type, b.reason, b.starts_at, b.ends_at, b.created_at,
              b.ip_address,
              target.public_id AS user_public_id,
       ...
       WHERE (
         $1::timestamptz IS NULL
         OR (b.created_at, b.id) < ($1::timestamptz, $2::bigint)
       )
       ORDER BY b.created_at DESC, b.id DESC
       LIMIT $3`,
      [cursor?.createdAt || null, cursor?.id || null, limit + 1]
  ...
  app.get('/admin', requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    const [users, reports, price, adminAccount] = await Promise.all([
      db.query(`SELECT public_id, display_alias, display_name, email, role, plan,
                       email_verified_at, admin_2fa_enabled_at, created_at
                FROM users WHERE deleted_at IS NULL
                ORDER BY created_at DESC, id DESC LIMIT 50`),
      db.query(`SELECT r.*, reporter.display_name AS reporter_name, reported.display_name AS reported_name FROM reports r LEFT JOIN users reporter ON reporter.id = r.reporter_user_id LEFT JOIN users reported ON reported.id = r.reported_user_id ORDER BY r.created_at DESC, r.id DESC LIMIT 50`),
