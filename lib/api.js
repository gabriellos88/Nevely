...
function registerApiRoutes(app, db, presence, options = {}) {
  ...
  app.get('/api/users/:id/profile', ...auth, asyncRoute(async (req, res) => {
    ...
    if (req.query.context === 'history') {
      const conversationId = asId(req.query.conversationId);
  ...
  app.get('/api/conversations', asyncRoute(async (req, res) => {
    ...
    const [result, unread] = await Promise.all([
      db.query(
      `SELECT c.id, c.public_id, c.type, c.status, c.started_at, c.ended_at, c.expires_at,
  ...
  app.get('/api/saved-chats', asyncRoute(async (req, res) => {
    ...
    if (!principal) return;
    const result = await db.query(
      `SELECT s.conversation_id, c.public_id AS conversation_public_id,
