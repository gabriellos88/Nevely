...
const { cleanPublicId, createPublicId, isPublicId } = require('./public-identifiers');
...
async function userIdFromPublicId(executor, value) {
  ...
  const result = await executor.query(
    `SELECT id FROM users
     WHERE ${canonical ? 'public_id' : 'legacy_public_id'} = $1 AND deleted_at IS NULL`,
    [publicId]
  );
    ...
    try {
      ...
      await client.query(
        `UPDATE users
         ...
         WHERE id = $1`,
      [accountUserId, copy.common.deletedUser, crypto.randomBytes(32).toString('hex'), createPublicId('user')]
      );
  ...
  app.get('/api/admin/users/:id', requireAuth, requireAdmin, asyncRoute(async (req, res) => {
    ...
    const result = await db.query(
      `SELECT u.public_id, u.username, u.display_name, u.email, u.role, u.plan,
       ...
       WHERE (u.public_id = $1 OR u.legacy_public_id = $1) AND u.deleted_at IS NULL`,
      [publicId]
    );
    if (!result.rowCount) return res.status(404).json({ error: copy.errors.userNotFound });
    const row = result.rows[0];
    ...
    try {
      ...
      await client.query(
        `UPDATE users SET username = 'deleted_' || id, display_name = $2,
             ...
             deleted_at = NOW(), updated_at = NOW()
         WHERE id = $1`,
        [targetUserId, copy.common.deletedUser, crypto.randomBytes(32).toString('hex'), createPublicId('user')]
      );
