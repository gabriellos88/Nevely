  ...
  app.post('/api/friend-requests', ...auth, requireVerifiedEmail, asyncRoute(async (req, res) => {
    ...
    try {
      ...
      await client.query(
        ...
        [[senderId, receiverId].sort((a, b) => a - b)]
      );
      const existing = await client.query(
      ...
      if (existing.rowCount) {
        await client.query('COMMIT');
        return res.json({ requestId: existing.rows[0].id });
      }
      ...
      await client.query(
        `INSERT INTO notifications (user_id, type, title, body, data)
          VALUES ($1, 'friend_request', $2, $3,
                  jsonb_build_object('requestId', $4::bigint, 'userPublicId', $5::text))`,
      ...
      return res.status(201).json({ requestId: result.rows[0].id });
    } catch (error) {
      await client.query('ROLLBACK');
      throw error;
    } finally {
