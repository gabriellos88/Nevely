...
async function persistConversationMessage(executor, active, socketId, text, { validate } = {}) {
  const client = await executor.getClient();
  try {
    await client.query('BEGIN');
    await client.query('SELECT pg_advisory_xact_lock($1)', [active.conversationId]);
    if (typeof validate === 'function' && !(await validate(client))) {
      const error = new Error('conversation message authorization failed');
      error.code = 'CONVERSATION_MESSAGE_UNAVAILABLE';
      throw error;
    }
    ...
    `WITH new_message AS (
       ...
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id, public_id, conversation_id, sender_user_id, sender_guest_id, created_at
     ), new_receipts AS (
     ...
     SELECT public_id AS id, public_id, created_at,
