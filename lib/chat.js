...
function registerChat(io, db, presence, options = {}) {
  ...
  async function pairUsers(a, b, type = 'random', reservedConversationId = null, restored = false) {
    ...
    io.to(a.socketId).emit('matched', matchPayload(a, b, shared, conversationId, type, aCanAddFriend, restored));
    io.to(b.socketId).emit('matched', matchPayload(b, a, shared, conversationId, type, bCanAddFriend, restored));
    return conversationId;
  }
  ...
  function matchPayload(user, partner, shared, conversationId, type, canAddFriendValue, restored = false) {
    return {
