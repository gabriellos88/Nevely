    ...
    socket.on('direct-chat-response', async (payload = {}, acknowledge) => {
      ...
      try {
        ...
        if (current?.status === action) {
          ...
          client.release();
          client = null;
          return done({ ok: true, status: action, started: false });
        }
        ...
        await client.query(
          ...
          [action, requestPublicId, account.internalId]
        );
        await client.query('COMMIT');
        client.release();
        client = null;
        ...
        if (!senderSocket) return done({ ok: true, status: action, started: false });
        await queueMatchmakingTransition(async () => {
