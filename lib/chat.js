...
function registerChat(io, db, presence, options = {}) {
  ...
  async function pairUsers(a, b, type = 'random', reservedConversationId = null, restored = false) {
    ...
    activePairs.set(a.socketId, { partnerId: b.socketId, conversationId, type, user: a, partner: b });
    activePairs.set(b.socketId, { partnerId: a.socketId, conversationId, type, user: b, partner: a });
    ...
    io.to(a.socketId).emit('matched', matchPayload(a, b, shared, conversationId, type, aCanAddFriend, aCanBlock, restored));
    io.to(b.socketId).emit('matched', matchPayload(b, a, shared, conversationId, type, bCanAddFriend, bCanBlock, restored));
    return conversationId;
  }
  ...
  function matchPayload(user, partner, shared, conversationId, type, canAddFriendValue, canBlockValue, restored = false) {
    return {
  ...
  async function restoreDirectConversation(socket) {
    ...
    try {
      ...
      await queueMatchmakingTransition(async () => {
        ...
        removeFromWaiting(partnerSocketId);
        await pairUsers(
  ...
  io.on('connection', (socket) => {
    ...
    if (account?.internalId && account.emailVerified) {
      setImmediate(() => void restoreDirectConversation(socket));
    }
    ...
    socket.on('find-partner', (payload = {}) => {
      queueSocketTransition(socket, async () => {
        ...
        try {
          ...
          if (startingPair?.type === 'direct') {
            socket.emit('chat-error', { message: copy.errors.directChatActive });
            return;
          }
          if (startingPair) {
        ...
        if (active.type === 'direct' && db.isConfigured) {
          ...
          if (!reservation.rowCount) {
            await queueMatchmakingTransition(() => {
              ...
              activePairs.delete(active.partnerId);
              io.to(socket.id).emit('partner-left', { conversationId: active.conversationId });
              io.to(active.partnerId).emit('partner-left', { conversationId: active.conversationId });
              notifyIdle();
            });
    ...
    socket.on('end-direct-chat', (acknowledge) => {
      const done = typeof acknowledge === 'function' ? acknowledge : () => {};
      queueSocketTransition(socket, async () => {
        try {
          const current = activePairs.get(socket.id);
          if (!current) {
            ...
            presence.emitToUser(Number(reservation.rows[0].partner_user_id), 'partner-left', {
              conversationId: Number(reservation.rows[0].conversation_id)
            });
      ...
      try {
        ...
        await client.query(
          `INSERT INTO notifications (user_id, type, title, body, data)
           VALUES ($1, 'chat_request', $2, $3, jsonb_build_object('requestPublicId', $4::text))`,
          [
            receiverId,
    ...
    socket.on('direct-chat-response', async (payload = {}, acknowledge) => {
      ...
      try {
        ...
        await queueMatchmakingTransition(async () => {
          const senderSocketId = action === 'accepted'
            ? presence.getSockets(senderId).find((id) => !activePairs.has(id))
            : null;
          ...
          if (action === 'accepted') {
            ...
            if (!eligibility.rows[0]?.allowed
                || !senderSocket?.connected || !socket.connected
                || activePairs.has(senderSocketId) || activePairs.has(socket.id)) {
              await client.query('ROLLBACK');
              client.release();
              client = null;
              done({ ok: false, error: copy.errors.chatRequestUnavailable });
            ...
            senderProfile = profileForSocket(senderSocket);
    ...
    async terminateDirectPair(firstUserId, secondUserId) {
      await queueMatchmakingTransition(async () => {
        ...
        for (const [socketId, active] of activePairs) {
          ...
          activePairs.delete(socketId);
          activePairs.delete(active.partnerId);
          io.to(socketId).emit('partner-left', { conversationId: active.conversationId });
          io.to(active.partnerId).emit('partner-left', { conversationId: active.conversationId });
        }
        notifyIdle();
      });
