    ...
    if (db.isConfigured) {
      ...
      db.query(
        `SELECT 1 FROM bans
         WHERE starts_at <= NOW()
           AND (
             (user_id = $1 AND (type = 'permanent' OR ends_at > NOW()))
             OR (
               type = 'ip'
               AND ip_address = $2
         ...
         LIMIT 1`,
        [account?.internalId || null, ipAddress]
      ).then((result) => {
        if (result.rowCount) {
          socket.emit('account-banned', { type: 'blocked' });
          socket.disconnect(true);
        }
      }).catch((error) => log.error('chat.socket_ban_check_failed', error));
    }
    ...
    socket.on('messages-read', async (payload = {}, acknowledge) => {
      ...
      try {
        const result = await db.query(
          ...
          [account.internalId, conversationId, upToMessageId]
        );
    ...
    socket.on('report', async (payload = {}) => {
      ...
      try {
        ...
        const report = await client.query(
          `INSERT INTO reports
             (reporter_user_id, reported_user_id, reporter_socket_id,
              reported_socket_id, conversation_id, reason, details)
    ...
    socket.on('direct-chat-request', async (payload = {}) => {
      ...
      try {
        const receiver = await db.query(
          ...
          [String(payload.publicId || payload.userId || '').slice(0, 40)]
        );
