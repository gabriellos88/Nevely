...
function registerChat(io, db, presence, options = {}) {
  ...
  async function pairUsers(a, b, type = 'random', reservedConversationId = null) {
    ...
    io.to(a.socketId).emit('matched', matchPayload(a, b, shared, conversationId, type, aCanAddFriend));
    io.to(b.socketId).emit('matched', matchPayload(b, a, shared, conversationId, type, bCanAddFriend));
    return conversationId;
  }
  ...
  function matchPayload(user, partner, shared, conversationId, type, canAddFriendValue) {
    return {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('end-direct-chat', (acknowledge) => {
      ...
      queueSocketTransition(socket, async () => {
        try {
          const current = activePairs.get(socket.id);
          if (!current) return done({ ok: true, ended: false });
