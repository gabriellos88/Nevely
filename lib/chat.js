  ...
  io.use(async (socket, next) => {
    ...
    try {
      ...
      const [accountBan, networkBan, guestBan, deviceBan] = await Promise.all([
        ...
        isNetworkBlocked(socket.handshake.address),
        guestId ? isGuestBlocked(guestId) : false,
        guestId ? isGuestDeviceRestricted(guestId) : false
      ]);
      if (accountBan.rowCount || networkBan || guestBan || deviceBan) return next(new Error('connection restricted'));
      ...
      socket.use(async (_packet, next) => {
        try {
          ...
          if (accountBan.rowCount || networkBan || guestBan || deviceBan) {
            socket.emit((guestBan || deviceBan) ? 'guest-restricted' : 'account-banned', (guestBan || deviceBan) ? {} : { type: 'blocked' });
