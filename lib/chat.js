...
const { appendAudit } = require('./moderation');
...
function registerChat(io, db, presence, options = {}) {
  ...
  async function markConversationEnded(conversationId, auditEntry = null) {
    ...
    try {
      ...
      if (ended.rowCount > 0 && auditEntry) {
        await appendAudit(client, {
          actorUserId: auditEntry.actorUserId,
          targetUserId: auditEntry.targetUserId,
          targetType: 'conversation',
          action: 'social.direct_conversation_ended',
          before: { conversationId: Number(conversationId), status: 'active' },
          after: { conversationId: Number(conversationId), status: 'ended' }
        });
      }
      await client.query('COMMIT');
      return true;
  ...
  async function endPair(socketId, eventForBoth = null, auditEntry = null) {
    ...
    const persisted = markConversationEnded(active.conversationId, auditEntry);
  ...
  async function directMessageContext(socket, conversationPublicId) {
    const accountId = Number(socket.request.session?.user?.internalId);
    if (!db.isConfigured || !accountId || !isPublicId(conversationPublicId, 'conversation')) return null;
    const result = await db.query(
      `SELECT c.id AS conversation_id, c.public_id AS conversation_public_id,
              partner.id, partner.public_id, partner.display_name, partner.profile_image_url,
              partner.age, partner.gender, partner.country, partner.plan, partner.email_verified_at
       FROM direct_conversation_pairs pair
       JOIN conversations c ON c.id = pair.conversation_id
       JOIN users owner ON owner.id = $2
       JOIN users partner ON partner.id = CASE
         WHEN pair.user_low_id = $2 THEN pair.user_high_id ELSE pair.user_low_id END
       WHERE c.public_id = $1
         AND (pair.user_low_id = $2 OR pair.user_high_id = $2)
         AND c.type = 'direct' AND c.status = 'active' AND c.deleted_for_everyone_at IS NULL
         AND owner.deleted_at IS NULL AND owner.email_verified_at IS NOT NULL
         AND partner.deleted_at IS NULL AND partner.email_verified_at IS NOT NULL
         AND NOT EXISTS (
           SELECT 1 FROM account_bans b
           WHERE b.user_id IN (owner.id, partner.id) AND b.revoked_at IS NULL AND b.starts_at <= NOW()
             AND (b.type = 'permanent' OR b.ends_at > NOW())
         )
         AND EXISTS (
           SELECT 1 FROM friendships f
           WHERE f.user_id = owner.id AND f.friend_id = partner.id
         )
         AND NOT EXISTS (
           SELECT 1 FROM blocked_users b
           WHERE (b.blocker_user_id = owner.id AND b.blocked_user_id = partner.id)
              OR (b.blocker_user_id = partner.id AND b.blocked_user_id = owner.id)
         )
       LIMIT 1`,
      [conversationPublicId, accountId]
    );
    ...
    return {
      ...
      type: 'direct',
      user: profileForSocket(socket),
      partner: profileForDirectAccount(row)
    };
  }

  async function validateDirectMessageParticipant(client, conversationId, accountId) {
    const result = await client.query(
      `SELECT c.id
       ...
       FOR UPDATE OF c, owner, partner`,
      [conversationId, accountId]
    );
    return result.rowCount === 1;
  }

  async function notifyDirectMessageBacklog(socket) {
    const accountId = Number(socket.request.session?.user?.internalId);
    if (!db.isConfigured || !accountId || !socket.connected) return;
    try {
      const pending = await db.query(
        `SELECT 1
         ...
         JOIN direct_conversation_pairs pair ON pair.conversation_id = c.id
         WHERE receipt.user_id = $1 AND receipt.read_at IS NULL
           AND m.sender_user_id IS DISTINCT FROM $1
           AND c.type = 'direct' AND c.status = 'active' AND c.deleted_for_everyone_at IS NULL
           AND (pair.user_low_id = $1 OR pair.user_high_id = $1)
         LIMIT 1`,
        [accountId]
      );
      if (pending.rowCount && socket.connected) socket.emit('direct-messages-available', {});
    } catch (error) {
      log.error('chat.direct_backlog_check_failed', error);
    }
  }

  async function persistMessage(active, socketId, text, options = {}) {
    if (!db.isConfigured || !active.conversationId) return null;
    return persistConversationMessage(db, active, socketId, text, options);
  }
  ...
  io.on('connection', (socket) => {
    ...
    if (account?.internalId && account.emailVerified) {
      setImmediate(() => void notifyDirectMessageBacklog(socket));
    }
    ...
    socket.on('send-message', async (rawPayload, acknowledge) => {
      ...
      const payload = rawPayload && typeof rawPayload === 'object'
        ? rawPayload
        : { text: rawPayload };
      const requestedConversationId = cleanPublicId(payload.conversationId);
      let active = activePairs.get(socket.id) || null;
      const text = cleanText(payload.text, MAX_MESSAGE_LENGTH);
      if (!text) return reply('message-error', { message: copy.errors.messageSend });
      try {
        if (active && requestedConversationId && active.conversationPublicId !== requestedConversationId) {
          return reply('message-error', { message: copy.errors.messageSend });
        }
        if (!active && requestedConversationId) {
          active = await directMessageContext(socket, requestedConversationId);
        }
        if (!active) return reply('message-error', { message: copy.errors.messageSend });
        ...
        const stored = await persistMessage(active, socket.id, text, {
          validate: active.type === 'direct'
            ? (client) => validateDirectMessageParticipant(client, active.conversationId, active.user.userId)
            : undefined
        });
        const deliveryPayload = { id: stored?.id || null, text, createdAt: stored?.created_at || new Date().toISOString() };
        if (active.type === 'direct') {
          presence.getSockets(active.partner.userId).forEach((partnerSocketId) => {
            const partnerActive = activePairs.get(partnerSocketId);
            if (partnerActive?.type === 'direct'
                && Number(partnerActive.conversationId) === Number(active.conversationId)) {
              io.to(partnerSocketId).emit('receive-message', deliveryPayload);
            }
          });
        } else {
          io.to(active.partnerId).emit('receive-message', deliveryPayload);
        }
        ...
        try {
          ...
          if (!current) {
            ...
            const persisted = await markConversationEnded(Number(reservation.rows[0].conversation_id), {
              actorUserId: Number(account.internalId),
              targetUserId: Number(reservation.rows[0].partner_user_id)
            });
          ...
          await queueMatchmakingTransition(async () => {
            ...
            ended = Boolean(await endPair(socket.id, null, {
              actorUserId: Number(account.internalId),
              targetUserId: Number(current.partner.userId)
            }));
          });
