...
const { purgeDueAccountsBatch } = require('./account-lifecycle');
                ...
                SELECT c2.id,
                        ROW_NUMBER() OVER (
                          PARTITION BY COALESCE(
                            'user:' || cp.user_id::text,
                            'guest:' || cp.guest_id::text
                          )
...
const BOUNDED_DELETE_POLICIES = [
  {
    ...
    sql: DELETE_EXPIRED_PENDING_REGISTRATIONS_SQL,
    params: (batchSize) => [batchSize, null, null, null]
  },
  {
    name: 'guestPrincipals',
    sql: `DELETE FROM guest_principals
          WHERE id IN (
             SELECT id FROM guest_principals
             WHERE retention_until <= NOW()
               AND NOT EXISTS (
                 SELECT 1 FROM saved_chats
                 WHERE saved_chats.guest_id = guest_principals.id
               )
  ...
  {
    name: 'moderationRateWindows',
    sql: `DELETE FROM moderation_rate_windows
          WHERE (principal_type, principal_id, action) IN (
            SELECT principal_type, principal_id, action
            FROM moderation_rate_windows
            WHERE expires_at <= NOW()
              AND (escalation_expires_at IS NULL OR escalation_expires_at <= NOW())
            ORDER BY expires_at, principal_type, principal_id, action LIMIT $1
          )`
  },
  {
    name: 'securityEvents',
  ...
  for (const policy of BOUNDED_DELETE_POLICIES) {
    for (let batchNumber = 0; batchNumber < maxBatchesPerPolicy; batchNumber += 1) {
      const result = await client.query(
        policy.sql,
        policy.params ? policy.params(batchSize) : [batchSize]
      );
...
async function drainAccountPurges(client, config) {
  let purged = 0;
  for (let batchNumber = 0; batchNumber < config.maxBatchesPerPolicy; batchNumber += 1) {
    const count = await purgeDueAccountsBatch(client, {
...
function createRetentionWorker({
  ...
  async function execute() {
    ...
    try {
      ...
      deletedCounts.accountPiiPurges = await drainAccountPurges(client, config);
      addCounts(
        deletedCounts,
