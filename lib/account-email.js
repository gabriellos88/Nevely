...
const PURPOSES = {
  verify_email: {
    ...
    action: 'Verify email',
    detail: 'This single-use link expires after one hour.'
  },
  password_reset: {
    expiresMs: 60 * 60 * 1000,
    path: '/reset-password',
  ...
  password_setup: {
    expiresMs: 60 * 60 * 1000,
    path: '/add-password',
    subject: 'Add a Nevely password',
    action: 'Add a Nevely password',
    detail: 'Google will remain connected. This single-use link expires after one hour.'
  },
  email_change: {
    expiresMs: 60 * 60 * 1000,
...
async function queueAccountEmail(executor, {
  ...
  const htmlBody = `<h1>${escapeHtml(configuration.subject)}</h1><p>${escapeHtml(configuration.detail)}</p><p><a href="${safeLink}">${escapeHtml(configuration.action)}</a></p><p>If you did not request this, you can ignore it.</p>`;
  await executor.query(
