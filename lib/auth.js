...
function sendAuthError(req, res, status, message, mode, environment) {
...
async function establishSession(req, row, { twoFactorVerified = false } = {}) {
...
function registerAuthRoutes(app, db, options = {}) {
  ...
  app.get('/register', (req, res) => {
    ...
    return res.render('auth-stub', authViewModel(req, 'register', {}, environment));
  });

  app.get('/login', (req, res) => {
    if (req.session?.user) return res.redirect('/chat');
    return res.render('auth-stub', authViewModel(req, 'login', {}, environment));
  });
  ...
  app.post('/register', databaseRequired, async (req, res, next) => {
    ...
    if (username.length < 3) {
      return sendAuthError(req, res, 400, copy.errors.usernameLength, 'register', environment);
    }
    if (!isValidEmail(email)) {
      return sendAuthError(req, res, 400, copy.errors.emailInvalid, 'register', environment);
    }
    if (password.length < 8 || password.length > 72) {
      return sendAuthError(req, res, 400, copy.errors.passwordLength, 'register', environment);
    }
    if (profile.error) {
      return sendAuthError(req, res, 400, profile.error, 'register', environment);
    }
    ...
    try {
      ...
      const result = await client.query(
        `INSERT INTO users
           (username, email, password_hash, public_id, display_alias, display_name,
            birth_date, gender, country, country_code, profile_completed_at, last_ip)
         VALUES ($1, $2, $3, $4, $5, $1, $6, $7, $8, $9, NOW(), $10)
         RETURNING *`,
        [
          username,
          email,
      ...
      if (wantsJson(req)) return res.status(201).json({ user, verificationEmailQueued: true });
      return res.redirect('/chat');
    } catch (error) {
      await client.query('ROLLBACK').catch(() => {});
      if (error.code === '23505') {
        return sendAuthError(req, res, 409, copy.errors.identityTaken, 'register', environment);
      }
      return next(error);
    } finally {
      client.release();
  ...
  app.post('/login', databaseRequired, async (req, res, next) => {
    ...
    try {
      ...
      if (user.role === 'admin' && user.admin_2fa_enabled_at) {
        await regenerateSession(req);
        req.session.pendingAdminTwoFactorUserId = Number(user.id);
  ...
  app.post('/login/2fa', databaseRequired, async (req, res, next) => {
    ...
    try {
      ...
      const safeUser = await establishSession(req, user, { twoFactorVerified: true });
  ...
  app.get('/complete-profile', requireAuth, databaseRequired, async (req, res, next) => {
    try {
  ...
  app.post('/complete-profile', requireAuth, databaseRequired, async (req, res, next) => {
    const profile = normalizeRegisteredProfile(req.body);
    if (profile.error) {
  ...
  app.post('/auth/google', databaseRequired, async (req, res, next) => {
    ...
    try {
      ...
      if (identity.rowCount) {
        ...
        const safeUser = await establishSession(req, user, {
          twoFactorVerified: user.role === 'admin' && Boolean(user.admin_2fa_enabled_at)
        });
      ...
      const profile = normalizeRegisteredProfile(req.body);
      const username = cleanText(req.body.username, 30);
      if (profile.error || username.length < 3) {
        return res.status(422).json({
          error: copy.errors.googleRegistrationProfileRequired,
          code: 'GOOGLE_PROFILE_REQUIRED'
        });
      }
      const hadGuestProfile = Boolean(req.session.guestProfile);
      const client = await db.getClient();
      try {
        await client.query('BEGIN');
        const created = await client.query(
          `INSERT INTO users
           ...
           VALUES ($1, $2, NULL, $3, $4, $5, $6, $7, $8, $9, NOW(), NOW(), $10, $11)
           RETURNING *`,
          [
            username,
            google.email,
            makePublicId(),
            makeDisplayAlias(),
            cleanText(google.name, 40) || username,
            profile.birthDate,
            profile.gender,
            profile.country,
            profile.countryCode,
        ...
        return res.status(201).json({ user: safeUser, guestMerged: hadGuestProfile });
      } catch (error) {
        await client.query('ROLLBACK').catch(() => {});
        throw error;
      } finally {
  ...
  app.post('/api/admin/reauth', requireAuth, databaseRequired, async (req, res, next) => {
    try {
      ...
      const secret = user.admin_totp_secret
        ? decryptSecret(user.admin_totp_secret, totpEncryptionKey)
        : null;
      if (!passwordValid || !secret || !verifyTotp(secret, req.body.code)) {
