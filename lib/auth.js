...
function registerAuthRoutes(app, db) {
  ...
  app.get('/register', (req, res) => {
    ...
    return res.render('auth-stub', {
      pageTitle: copy.pageTitles.register, mode: 'register', error: null, values: {}
    });
  });
  ...
  app.get('/login', (req, res) => {
    if (req.session?.user) return res.redirect('/chat');
    return res.render('auth-stub', {
      pageTitle: copy.pageTitles.login, mode: 'login', error: null, values: {}
    });
  });

  app.post('/register', async (req, res, next) => {
    if (!db.isConfigured) return res.status(503).send(copy.errors.serviceUnavailable);
    const username = cleanText(req.body.username, 30);
    const email = normalizeEmail(req.body.email);
    const password = typeof req.body.password === 'string' ? req.body.password : '';

    if (username.length < 3) return sendAuthError(req, res, 400, copy.errors.usernameLength, 'register');
    if (!isValidEmail(email)) return sendAuthError(req, res, 400, copy.errors.emailInvalid, 'register');
    if (password.length < 8 || password.length > 72) return sendAuthError(req, res, 400, copy.errors.passwordLength, 'register');
    ...
    try {
      ...
      if (error.code === '23505') return sendAuthError(req, res, 409, copy.errors.identityTaken, 'register');
      return next(error);
    }
  });
  ...
  app.post('/login', async (req, res, next) => {
    ...
    try {
      ...
      if (!valid) return sendAuthError(req, res, 401, copy.errors.credentialsIncorrect, 'login');
      ...
      if (ban.rowCount) return sendAuthError(req, res, 403, copy.errors.accountSuspended, 'login');
