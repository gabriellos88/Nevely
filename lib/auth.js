...
function registerAuthRoutes(app, db, options = {}) {
  ...
  app.get('/suspension', databaseRequired, async (req, res, next) => {
    try {
      ...
      return res.render('suspension', {
        ...
        suspension: suspensionPayload(ban).suspension,
        canAppeal: ban.source === 'account' && Boolean(ban.id),
        csrfToken: res.locals.csrfToken
      });
    } catch (error) {
      return next(error);
  ...
  app.post('/api/suspension/appeals', databaseRequired, async (req, res, next) => {
    const suspension = req.session?.suspension;
    if (!suspension?.userId || !suspension.accountBanId) {
      return res.status(401).json({ error: 'An active account suspension is required.' });
    }
    let appealText;
    try {
      appealText = requiredReason(req.body.reason);
    } catch (error) {
    ...
    try {
      ...
      const ban = await client.query(
        `SELECT id, user_id FROM account_bans
         WHERE id = $1 AND user_id = $2 AND revoked_at IS NULL AND starts_at <= NOW()
           AND (type = 'permanent' OR ends_at > NOW()) FOR UPDATE`,
        [suspension.accountBanId, suspension.userId]
      );
      if (!ban.rowCount) {
        const unavailable = new Error('The account suspension is no longer active.');
        unavailable.code = 'SUSPENSION_INACTIVE';
        throw unavailable;
      }
      const existing = await client.query(
        `SELECT id, created_at FROM moderation_appeals
         WHERE account_ban_id = $1 AND appellant_user_id = $2 AND status = 'pending'
         ORDER BY id DESC LIMIT 1 FOR UPDATE`,
        [suspension.accountBanId, suspension.userId]
      );
      if (existing.rowCount) {
        await client.query('COMMIT');
        return res.json({ appealId: Number(existing.rows[0].id), createdAt: existing.rows[0].created_at, idempotent: true });
      }
      const created = await client.query(
        `INSERT INTO moderation_appeals (account_ban_id, appellant_user_id, appeal_text)
         VALUES ($1, $2, $3) RETURNING id, created_at`,
        [suspension.accountBanId, suspension.userId, appealText]
      );
      await appendAudit(client, {
        actorUserId: suspension.userId, targetUserId: suspension.userId, targetType: 'account_ban',
        action: 'account_ban_appeal_created', reason: appealText,
        before: { pendingAppeal: false }, after: { appealId: Number(created.rows[0].id), banId: Number(suspension.accountBanId) }, correlationId
      });
      await client.query('COMMIT');
      return res.status(201).json({ appealId: Number(created.rows[0].id), createdAt: created.rows[0].created_at, idempotent: false });
    } catch (error) {
      await client.query('ROLLBACK').catch(() => {});
      if (error.code === '23505') {
      ...
      if (error.code === 'SUSPENSION_INACTIVE') return res.status(409).json({ error: error.message, code: error.code });
      return next(error);
    } finally {
      client.release();
    }
